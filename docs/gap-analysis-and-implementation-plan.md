...
10. **Add a server-side embeddings path.** Give embeddings the same shared-key proxy chat has (an `embed` edge function using the admin key), so signed-in no-key users actually get grounded RAG instead of silent empty hits. _(3d)_
    **Status (this PR):** Implemented. `supabase/functions/embed/` proxies query embeddings for any signed-in caller (`callerFrom` JWT check; indexing stays admin/BYOK-only, per D2). A shared `supabase/functions/_shared/embedProvider.ts` resolver (embeddings-row → chat-provider → env OpenAI key) is now the single source of truth for both `embed` and `content-index` (the latter converged off its old hardcoded OpenAI path, so query- and index-side vectors can't drift). A dedicated `ai_provider_config key='embeddings'` row is used because the documented chat provider (OpenRouter) has **no** `/embeddings` endpoint (D1); the admin editor is `EmbeddingsProviderCard` in `src/components/admin/SecretsSection.tsx`. Client `embedQuery` (`src/services/embeddings.ts`) routes BYOK→provider directly, keyless-signed-in→proxy; `embed()` still throws on failure (never silent-empty).
