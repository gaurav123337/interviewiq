...
At ${t} level, expect the candidate to: ${N(t)}

Tie everything back to how they'd explain it in a 45-minute whiteboard interview.${r}`}async function B(e,t){if(!m())throw new Error("AI coaching is temporarily disabled.");if(f()&&w()<=0)throw new Error("You've used your free AI coaching for today — upgrade to Pro for unlimited.");const s=y(t.fieldId),a=p(t.targetLevel),r=u(e),n=k(e,a.name,(s==null?void 0:s.name)??"",r),c=`Teach the system design topic "${e}" for a ${a.name} ${(s==null?void 0:s.name)??""} interview.
Include:
