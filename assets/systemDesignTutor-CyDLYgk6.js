...
At ${t} level, expect the candidate to: ${_(t)}

Tie everything back to how they'd explain it in a 45-minute whiteboard interview.${r}`}async function S(e,t){if(!m())throw new Error("AI coaching is temporarily disabled.");if(f()&&w()<=0)throw new Error("You've used your free AI coaching for today — upgrade to Pro for unlimited.");const s=y(t.fieldId),a=p(t.targetLevel),r=u(e),n=k(e,a.name,(s==null?void 0:s.name)??"",r),c=`Teach the system design topic "${e}" for a ${a.name} ${(s==null?void 0:s.name)??""} interview.
Include:
...
If this is a known pattern, include a concrete architecture walkthrough.`),{sys:d}=await g(n,e,{field:t.fieldId,level:t.targetLevel}),l=await b("tutor",[{role:"system",content:d},{role:"user",content:c}],{maxTokens:800});return x(),v("ai_call",{system_design:!0,topic:e.slice(0,100)}),l}async function Y(e,t,s){var h;if(!m())throw new Error("AI coaching is temporarily disabled.");if(f()&&w()<=0)throw new Error("You've used your free AI coaching for today — upgrade to Pro for unlimited.");const a=y(t.fieldId),r=p(t.targetLevel),n=u(e),c=k(e,r.name,(a==null?void 0:a.name)??"",n),d=((h=[...s].reverse().find(o=>o.role==="user"))==null?void 0:h.content)??"",{sys:l,citations:j,grounded:$,checked:E}=await g(c,d||e,{field:t.fieldId,level:t.targetLevel}),I=[{role:"system",content:l},...s.map(o=>({role:o.role,content:o.content}))],A=await b("tutor",I,{maxTokens:600});return x(),v("ai_call",{system_design_chat:!0,topic:e.slice(0,100)}),{text:A,citations:j,grounded:$,checked:E}}export{P as D,S as e,B as i,Y as s};
