...
Tie everything back to how they'd explain it in a 45-minute whiteboard interview.${r}`}async function F(e,t){if(!m())throw new Error("AI coaching is temporarily disabled.");if(f()&&y()<=0)throw new Error("You've used your free AI coaching for today — upgrade to Pro for unlimited.");const s=w(t.fieldId),a=p(t.targetLevel),r=u(e),n=k(e,a.name,(s==null?void 0:s.name)??"",r),c=`Teach the system design topic "${e}" for a ${a.name} ${(s==null?void 0:s.name)??""} interview.
Include:
...
If this is a known pattern, include a concrete architecture walkthrough.`),{sys:d}=await g(n,e,{field:t.fieldId,level:t.targetLevel}),l=await x("tutor",[{role:"system",content:d},{role:"user",content:c}],{maxTokens:800});return b(),v("ai_call",{system_design:!0,topic:e.slice(0,100)}),l}async function S(e,t,s){var h;if(!m())throw new Error("AI coaching is temporarily disabled.");if(f()&&y()<=0)throw new Error("You've used your free AI coaching for today — upgrade to Pro for unlimited.");const a=w(t.fieldId),r=p(t.targetLevel),n=u(e),c=k(e,r.name,(a==null?void 0:a.name)??"",n),d=((h=[...s].reverse().find(o=>o.role==="user"))==null?void 0:h.content)??"",{sys:l,citations:j,grounded:E,checked:$}=await g(c,d||e,{field:t.fieldId,level:t.targetLevel}),A=[{role:"system",content:l},...s.map(o=>({role:o.role,content:o.content}))],I=await x("tutor",A,{maxTokens:600});return b(),v("ai_call",{system_design_chat:!0,topic:e.slice(0,100)}),{text:I,citations:j,grounded:E,checked:$}}export{P as D,F as e,B as i,S as s};
